...
namespace RefreshVIR
{
    internal static class PowerBiPublishService
    {
            ...
            IProgress<string>? progress = null)
        {
            ...
            await PublishPbixAsync(session, workspaceId, pbixPath, datasetId: null, progress);
        }
            ...
            IProgress<string>? progress = null)
        {
            ...
            RefreshScheduleSnapshot? savedRefreshSchedule = null;
            if (datasetId is Guid targetDatasetId && targetDatasetId != Guid.Empty)
            {
                progress?.Report("Ütemezett frissítés ideiglenes szüneteltetése...");
                savedRefreshSchedule = await PowerBiDatasetApi.TryDisableRefreshScheduleForPublishAsync(
                    httpClient,
                    workspaceId,
                    targetDatasetId);
            }
            ...
            try
            {
                await UploadPbixAsync(httpClient, workspaceId, pbixPath, fileName, publishContext, progress);
            }
            finally
            {
                if (datasetId is Guid restoreDatasetId
                    && restoreDatasetId != Guid.Empty
                    && savedRefreshSchedule != null)
                {
                    try
                    {
                        progress?.Report("Ütemezett frissítés visszaállítása...");
                        await PowerBiDatasetApi.RestoreRefreshScheduleAfterPublishAsync(
                            httpClient,
                            workspaceId,
                            restoreDatasetId,
                            savedRefreshSchedule);
                    }
                    catch (Exception ex)
                    {
                        throw new InvalidOperationException(
                            "A PBIX publikálás sikerült, de az ütemezett frissítés visszaállítása sikertelen. " +
                            "Ellenőrizd a dataset refresh ütemezését a Power BI portálon.",
                            ex);
                    }
                }
            }
        ...
        private static async Task UploadPbixAsync(
